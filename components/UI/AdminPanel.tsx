...
import { PromoCode, GameResult, User } from '../../types';
...
type Tab = 'dashboard' | 'requests' | 'codes';
  ...
  return (
    <div className="w-full h-full bg-slate-50 overflow-hidden flex flex-col">
      ...
      <div className="p-8 pb-0 max-w-7xl mx-auto w-full flex flex-col md:flex-row justify-between items-center gap-6">
        <div>
           <h1 className="text-3xl font-thin text-slate-800 uppercase italic leading-none tracking-tight">Admin Console</h1>
           <p className="text-[9px] font-bold text-slate-300 uppercase tracking-[0.4em] mt-2">Панель управления</p>
        </div>
        <div className="flex gap-4">
            <button onClick={onTestGame} className="bg-emerald-500 text-white px-6 py-3 rounded-2xl font-bold uppercase text-[9px] tracking-widest hover:bg-emerald-600 transition">Тест игры</button>
      ...
      <div className="px-8 mt-8 max-w-7xl mx-auto w-full flex gap-3 overflow-x-auto pb-2">
         <TabButton id="dashboard" label="Дашборд" />
         <TabButton id="requests" label="Заявки" count={stats.deliveryRequests} />
         <TabButton id="codes" label="Коды" count={codes.filter(c => !c.isUsed).length} />
      </div>
                ...
                <div className="space-y-8 animate-fade-in">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                        {[
                            ...
                            { label: 'Призы', val: stats.prizesAwarded.tier1 + stats.prizesAwarded.tier2 + stats.prizesAwarded.tier3 + stats.prizesAwarded.tier4 + stats.prizesAwarded.tier5 }
                        ].map((s, i) => (
                ...
                <div className="animate-fade-in">
                    <div className="bg-white rounded-[40px] shadow-sm border border-slate-100 overflow-hidden min-h-[50vh]">
                        <div className="p-8 border-b border-slate-50 flex justify-between items-center">
                            ...
                            <div className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest bg-emerald-50 px-3 py-1 rounded-lg">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-left">
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    ...
                                    {users.filter(u => u.deliveryRequested).length === 0 && (
                                        <tr>
                                            <td colSpan={5} className="text-center py-20 text-slate-300 font-bold uppercase text-[10px] tracking-widest">
                                                Нет активных заявок
                                            </td>
                                        </tr>
                                    )}
                            ...
                            <table className="w-full text-left">
                                ...
                                <tbody className="divide-y divide-slate-50">
                                    {codes.map((c) => {
                                        ...
                                        const statusColor = c.isUsed ? 'bg-slate-100 text-slate-400' : (c.isIssued ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600');
                ...
                {userResults.length > 0 ? userResults.map((res) => (
                  <div key={res.id} className="p-8 bg-slate-50/50 rounded-[40px] flex justify-between items-center border border-slate-100">
                    <div>
                      ...
                      <div className={`font-bold text-sm ${res.prize ? 'text-emerald-500' : 'text-slate-500'}`}>
                        {res.prize || 'Без приза'}
                      </div>
